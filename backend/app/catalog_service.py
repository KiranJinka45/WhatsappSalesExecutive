...
def parse_and_sync_catalog(
    ...
    if rows_to_process:
        # Pre-fetch existing categories for tenant and create missing ones
        ...
        # Track existing SKUs for org to accurately report created vs updated counts
        existing_skus = {p.sku for p in db.query(models.Product.sku).filter(models.Product.organization_id == org_id).all()}

        values_list = []
        for row_idx, valid_row in rows_to_process:
            category = existing_cats[valid_row.category_name.lower()]
            values_list.append({
                "id": uuid.uuid4(),
        ...
        # Atomic PostgreSQL Bulk Upsert using INSERT ... ON CONFLICT (organization_id, sku) DO UPDATE
        stmt = insert(models.Product).values(values_list)
        update_set = {
            ...
            "video_urls": stmt.excluded.video_urls,
            "embedding_status": "pending",
            "updated_at": func.now()
        }
        upsert_stmt = stmt.on_conflict_do_update(
            index_elements=["organization_id", "sku"],
            set_=update_set
        ).returning(models.Product.id, models.Product.sku)

        res = db.execute(upsert_stmt)
        returned_rows = res.all()
        db.commit()

        for prod_id, prod_sku in returned_rows:
            returned_product_ids.append(str(prod_id))
            if prod_sku in existing_skus:
                products_updated += 1
            else:
                products_created += 1

    # Schedule vector embedding tasks in background AFTER single transaction commit
    for prod_id_str in returned_product_ids:
        background_tasks.add_task(generate_product_embedding_task, SessionLocal, prod_id_str)
