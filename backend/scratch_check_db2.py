...
failed_msgs = db.query(models.Message).filter(models.Message.status == 'failed').order_by(models.Message.created_at.desc()).limit(5).all()

with open("error_log_output2.txt", "w", encoding="utf-8") as f:
